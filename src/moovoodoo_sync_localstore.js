//-- Moovoodoo.Sync.LocalStore

// A simple module to replace `Moovoodoo.sync` with *localStorage*-based
...
Moovoodoo.sync.LocalStore = function(method, model, options) {
  ...
  switch (method) {
    ...
    case "destroy":  resp = store.destroy(model);                           break;
  }

  if (resp) {
    options.onSuccess(resp);
  } else {
    options.onError("Record not found");
  }
};