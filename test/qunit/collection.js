window.addEvent('domready', function() {
  ...
  module("Moovoodoo.Collection");

  var a         = new Moovoodoo.Model({id: 3, label: 'a'});
  var b         = new Moovoodoo.Model({id: 2, label: 'b'});
  var c         = new Moovoodoo.Model({id: 1, label: 'c'});
  var d         = new Moovoodoo.Model({id: 0, label: 'd'});
  var e         = null;
  var col       = new Moovoodoo.Collection([a,b,c,d]);
  var otherCol  = new Moovoodoo.Collection();
  ...
  test("Collection: get with non-default ids", function() {
    var col = new Moovoodoo.Collection();
    Moovoodoo.Model.idAttribute = '_id';
    var model = new Moovoodoo.Model({_id: 100});
    col.add(model);
  ...
  test("Collection: add model with attributes modified by set", function() {
    var CustomSetModel = new Class({
      ...
      set: function (attributes, options) {
        ...
        Moovoodoo.Model.prototype.set.call(this, attributes, options);
      }
    });

    var CustomSetCollection = new Class({
  ...
  test("Collection: add", function() {
    ...
    e = new Moovoodoo.Model({id: 10, label : 'e'});
    otherCol.add(e);
    otherCol.addEvent('add', function() {
      secondAdded = true;
    });
    col.addEvent('add', function(event){
      added = event.model.get('label');
      opts = event.options;
    });
    ...
    var f = new Moovoodoo.Model({id: 20, label : 'f'});
    var g = new Moovoodoo.Model({id: 21, label : 'g'});
    var h = new Moovoodoo.Model({id: 22, label : 'h'});
    var atCol = new Moovoodoo.Collection([f, g, h]);
  ...
  test("Collection: add model to collection and verify index updates", function() {
    ...
    col.addEvent('add', function(event) {
      counts.push(event.options.index);  
    });
    col.add(f); 
    col.add(g); 
    col.add(h); 
  ...
  test("Collection: add model to collection twice", function() {
    try {
      // no id, same cid
      var a2 = new Moovoodoo.Model({label: a.label});
      a2.cid = a.cid;
      col.add(a2);
  ...
  test("Collection: add model to multiple collections", function() {
    var counter = 0;
    var e = new Moovoodoo.Model({id: 10, label : 'e'});
    e.addEvent('add', function(event) {
      counter++;
      equals(e, event.model);
      if (counter > 1) {
        equals(event.collection, colF);
      } else {
        equals(event.collection, colE);
      }
    });
    var colE = new Moovoodoo.Collection([]);
    colE.addEvent('add', function(event) {
      equals(e, event.model);
      equals(colE, event.collection);
    });
    var colF = new Moovoodoo.Collection([]);
    colF.addEvent('add', function(event) {
      equals(e, event.model);
      equals(colF, event.collection);
    });
    colE.add(e);
  ...
  test("Collection: remove", function() {
    ...
    var e = new Moovoodoo.Model({id: 10, label : 'e'});
    col.add(e);
    col.addEvent('remove', function(event){ removed = event.model.get('label'); });
    otherCol.addEvent('remove', function(){ otherRemoved = true; });
    col.remove(e);
    equals(removed, 'e');
    equals(col.length, 4);
  ...
  test("Collection: remove should return correct index events", function() {
    ...
    var g = new Moovoodoo.Model({id: 21, label : 'g'});
    var h = new Moovoodoo.Model({id: 22, label : 'h'});
    var col = new Moovoodoo.Collection([f,g,h]);
    
    var counts = [];
     
    col.addEvent('remove', function(event) {
      counts.push(event.options.index);  
    });
    col.remove(h); 
    col.remove(g); 
  ...
  test("Collection: events are unbound on remove", function() {
    var counter = 0;
    var dj = new Moovoodoo.Model();
    var emcees = new Moovoodoo.Collection([dj]);
    emcees.addEvent('change', function(){ counter++; });
    dj.set({name : 'Kool'});
    equals(counter, 1);
    emcees.reset([]);
  ...
  /*test("Collection: remove in multiple collections", function() {
    ...
    var passed = false;
    var e = new Moovoodoo.Model(modelData);
    var f = new Moovoodoo.Model(modelData);
    f.addEvent('remove', function() {
      passed = true;
    });
    var colE = new Moovoodoo.Collection([e]);
    var colF = new Moovoodoo.Collection([f]);
    ok(e != f);
    ok(colE.length == 1);
    ok(colF.length == 1);
    colE.remove(e);
  ...
  test("Collection: remove same model in multiple collection", function() {
    var counter = 0;
    var e = new Moovoodoo.Model({id: 5, title: 'Othello'});
    e.addEvent('remove', function(model, collection) {
      counter++;
      equals(e, model);
      if (counter > 1) {
    ...
    var colE = new Moovoodoo.Collection([e]);
    colE.addEvent('remove', function(model, collection) {
      equals(e, model);
      equals(colE, collection);
    });
    var colF = new Moovoodoo.Collection([e]);
    colF.addEvent('remove', function(model, collection) {
      equals(e, model);
  ...
  test("Colllection: non-persisted model destroy removes from all collections", function() {
    var e = new Moovoodoo.Model({title: 'Othello'});
  ...
  test("Collection: create enforces validation", function() {
    var ValidatingModel = Moovoodoo.Model.extend({
      validate: function(attrs) {
        return "fail";
      }
    });
    var ValidatingCollection = Moovoodoo.Collection.extend({
  ...
  test("collection: initialize", function() {
    var Collection = Moovoodoo.Collection.extend({
      initialize: function() {
        this.one = 1;
      }
    });
  ...
  test("Collection: reset", function() {
    ...
    col.addEvent('reset', function() { resetCount += 1; });
    col.reset([]);
    equals(resetCount, 1);
    equals(col.length, 0);
  ...
  test("Collection: trigger custom events on models", function() {
    var fired = null;
    a.addEvent("custom", function() { fired = true; });
    a.trigger("custom");
    equals(fired, true);
  }); */
